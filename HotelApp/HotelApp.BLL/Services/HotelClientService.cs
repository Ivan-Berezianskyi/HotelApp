...
{
    public class HotelClientService : IHotelClientService
    {
        ...
        public IReadOnlyList<Room> GetMyOrders()
        {
            ...
            List<DbBooking> activeBookings = _dbContext.Bookings
                .AsNoTracking()
            ...
            foreach (DbBooking booking in activeBookings)
            {
                Room? room = _hotel.FindRoomByNumber(booking.RoomNumber);
                if (room != null)
                {
                    rooms.Add(room);
                }
